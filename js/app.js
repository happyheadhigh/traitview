...
function applyMispricedFilters(){
  ...
  const rankMap = (typeof RARITY_MODE !== 'undefined' && RARITY_MODE === 'theoretical' && typeof RARITY_THEO_RANK !== 'undefined' && RARITY_THEO_RANK.size) ? RARITY_THEO_RANK : (typeof RARITY_OBS_RANK !== 'undefined' ? RARITY_OBS_RANK : new Map());
  ...
  if(summaryRow && filtered.length){
    ...
    `;
  } else if(summaryRow){ summaryRow.style.display='none'; }
  // Re-render cards (sync — cards already built, just re-sort html)
  // Cards are already built in _mispricedRendered; just re-order them
...
let pinnedA=null, pinnedB=null, pinnedSet=[]; const OPEN_GROUPS=new Set();
let CHART_ID_MAP={};
...
function applyViewMode(val){
  ...
  // Force grid card styles via JS — belt-and-suspenders over CSS
  if(val === 'compact' || val === 'standard' || val === 'grid5'){
    ...
    tg.querySelectorAll('.token').forEach(card => {
      ...
      // Inject compact price badge if listed
    ...
    // List: reset inline styles but keep tmeta visible, CSS handles layout
    tg.querySelectorAll('.token').forEach(card => {
      card.style.cssText = '';
      const pb = card.querySelector('.pinbar');
      if(pb) pb.style.display = 'none'; // hide pinbar in list view
      const tm = card.querySelector('.tmeta');
      if(tm) tm.style.cssText = ''; // let CSS control
      const th = card.querySelector('.thumb');
      if(th) th.style.cssText = '';
      card.querySelectorAll('.compact-price-badge').forEach(b => b.remove());
    });
  } else {
    // Reset inline styles when leaving compact/grid modes
    tg.querySelectorAll('.token').forEach(card => {
      card.style.cssText = '';
      const pb = card.querySelector('.pinbar');
...
function getRankMapForSort(){ 
  // use current rarity mode map (observed or theoretical if available)
...
function getListingEth(id){
  ...
  if(!ent) return null;
  // price_eth is normalized float after fetchLiveForIds
  if(ent.price_eth != null) return Number(ent.price_eth);
  // fallback: parse from price field
  return parseEthMaybeWei(ent.price);
}
function getPriceSorted(ids, asc){
  // Sort by listing price. Unlisted tokens go to end.
...
function openMobileAnalytics(){
  ...
  const tabs = isMob
    ? ['chart','sales','mispriced','floor','holders','wallet']
    : ['chart','sales','mispriced','scatter','floor','holders','wallet'];
  const labels = {chart:'Traits',sales:'Sales',mispriced:'Mispriced',scatter:'Price vs Rank',floor:'Floor Trend',holders:'Holders',wallet:'Wallet'};
...
function switchAnalyticsSheetTab(name){
  ...
  const tabs = ['chart','sales','mispriced','scatter','floor','holders','wallet'];
...
function closeMobileAnalytics(){
  ...
  const body = document.getElementById('analyticsSheetBody');
  if(tabPanel){
    ['chart','sales','mispriced','scatter','floor','holders','wallet'].forEach(name => {
