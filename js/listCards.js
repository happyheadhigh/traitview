/* TraitView list/card rendering helpers.
 ...
 function listStatsRowHtml(id, rank, priceStr){
  ...
  // jv: "the recent sales for nekoads correctly displays the USDG price
  // and text but the live listings doesn't." This "Listed" cell was
  // WETH-aware (colors WETH red, everything else green) but still
  // hardcoded the "Ξ" glyph for anything that wasn't WETH specifically,
  // including USDG -- the same bug class already fixed in several other
  // spots, just never caught here since this function lives in its own
  // file (listCards.js) that an earlier sweep never touched. Using the
  // same shared priceGlyphAndSuffix() helper those other fixes settled
  // on, rather than another one-off inline check.
  ...
  return `<div class="vs-datarow">` +
    ...
    `<div class="vs-cell"><div class="vs-label">Listed</div><div class="vs-val ${listedCls}">${priceStr ? _pgs.glyph+priceStr+_pgs.suffix : '—'}</div></div>` +
