...
if(window.__TV_LANDING__){
  ...
  function reactionRowHtml(slug, expanded){
    ...
    // jv: "on mobile is where i applied the +1 for the emojis but on
    // desktop i cannot see them. Only viewable on mobile... the only
    // person seeing the emoji taps are the people who actually did them
    // themselves... They need to work for everyone 1 tap per emoji,
    // viewable on all devices." Confirmed real bug, not a caching issue:
    // this collapsed summary only ever rendered pills for `mine` -- the
    // emojis THIS specific browser had picked -- so a device that never
    // picked anything itself (a different browser viewing the same
    // reaction, or the connecting person's own second device) saw
    // nothing at all here, even though the real, shared count from
    // every device combined was sitting right there in `counts` the
    // whole time. Now shows every emoji with a nonzero count, not just
    // the ones this browser happens to own -- "mine" still gets its own
    // highlight color so your own pick(s) are still visually distinct,
    // but the underlying count itself is the same real, shared number
    // for anyone looking, on any device.
    const shown = REACTION_EMOJI.filter(e => (counts[e] || 0) > 0);
  ...
  const FAQ_ITEMS = [
    ...
    // jv: "Put in the faq how the bot and traitview are connected" --
    // expanded and moved up to second, right after "What is TraitView?".
    {
      q: 'How are TraitView and the Discord bot connected?',
      a: 'They\'re two halves of the same system, running on one shared database.<br><br>'
       + '<b>The bot does the work behind the scenes.</b> It pulls in every token\'s on-chain traits and keeps listings and sales synced around the clock. TraitView reads that same data, so what you see here is exactly what the bot is tracking right now.<br><br>'
       + '<b>In Discord,</b> the bot posts sales and listing alerts to your server and answers commands like /traitfind, /rankfind and /sweep. Every alert links straight back to that exact token here on TraitView.<br><br>'
       + '<b>Adding a collection works the same way.</b> Set it up in the bot with /setup, and once it\'s backfilled it shows up here automatically.',
    },
    {
      q: 'What is TV Rank, and how is it calculated?',
      a: 'TV Rank (▲) is TraitView\'s own rarity ranking, computed directly from a collection\'s actual trait data -- separate from OS Rank (◆), which is whatever OpenSea itself reports. Every trait a token has is scored by how rare that exact trait value is across the whole collection (a value only 12 tokens share scores far higher than one shared by 4,000). How many traits a token has factors in too, as its own separate signal -- an unusually bare or unusually loaded token gets credit for that on its own, the same way OpenSea\'s own ranking rewards it -- and a token with a truly one-of-a-kind trait value is guaranteed a spot at the very top no matter what else is on it. All of that combines into one rarity score per token; Rank #1 is the highest score, down to the most common. Burned tokens are excluded entirely and never receive a rank, since they\'re no longer part of the collection. If a collection has published its own intended/theoretical trait odds (from how it was originally generated) rather than relying purely on what actually got minted, TraitView can rank against those instead for extra precision. Toggle between TV and OS rank anywhere you see the ▲/◆ icon.',
    },
    {
  ...
  // jv: "is there a prompt on the landing page with a walk through on how to
  // install it as an app?" -> "Yes". The hamburger menu (with 'Install
  // app') doesn't exist on the landing page, so first-time visitors never
  // learned the app existed. Phones only; never inside the installed app;
  // dismissing hides it for 30 days. Lives inside #landingPage because the
  // landing page hides everything else in <body>. Install reuses
  // tvInstallApp() from app.js (Chrome's prompt on Android, the Share ->
  // Add to Home Screen guide on iPhone).
  (function installBanner(){
    const KEY = 'tvInstallBannerDismissedAt';
    const installed = (window.matchMedia && window.matchMedia('(display-mode: standalone)').matches) || window.navigator.standalone === true;
    const phone = window.matchMedia && window.matchMedia('(pointer: coarse)').matches && window._tvIsPhone();
    let dismissedAt = 0; try{ dismissedAt = +localStorage.getItem(KEY) || 0; }catch(_){}
    // jv: "I want to do a demo video and take screenshots... Anyway to get
    // that to show for me just 1 more time?" -> ?installdemo=1 forces the
    // banner (ignores dismissal and installed state) for demos/screenshots.
    const demo = new URLSearchParams(location.search).get('installdemo') === '1';
    if(!demo && (installed || !phone || Date.now() - dismissedAt < 30 * 86400000)) return;
    const host = document.getElementById('landingPage');
    if(!host || document.getElementById('tvInstallBanner')) return;
    const dismiss = () => { try{ localStorage.setItem(KEY, String(Date.now())); }catch(_){} bar.remove(); };
    const bar = document.createElement('div');
    bar.id = 'tvInstallBanner';
    bar.setAttribute('role', 'region');
    bar.setAttribute('aria-label', 'Install TraitView');
    bar.innerHTML = `<img src="/images/icons/icon-192.png" width="40" height="40" alt="">
      <div class="tvib-text"><b>Get TraitView on your home screen</b><span>Opens full-screen, like an app</span></div>
      <button type="button" class="tvib-install">Install</button>
      <button type="button" class="tvib-close" aria-label="Dismiss">✕</button>`;
    bar.querySelector('.tvib-install').addEventListener('click', () => {
      if(typeof tvInstallApp === 'function') tvInstallApp();
      dismiss();
    });
    bar.querySelector('.tvib-close').addEventListener('click', dismiss);
    window.addEventListener('appinstalled', () => bar.remove());
    // after the page has settled, so it never competes with first paint
    setTimeout(() => host.appendChild(bar), 1200);
  })();
}
