...
function _svgTextFromAny(src){
  ...
  if(!s) return null;
  if(s.startsWith('<svg') || s.startsWith('<?xml')) return _svgCrisp(s);
  if(s.startsWith('data:image/svg')){
    const comma = s.indexOf(',');
    if(comma < 0) return null;
    const meta = s.slice(5, comma).toLowerCase();
    const body = s.slice(comma + 1);
    try{ return _svgCrisp(meta.includes('base64') ? atob(body) : decodeURIComponent(body)); }catch(_){ return null; }
  }
  return null;
}
...
// viewBox -- which is essentially always true for a responsive,
// variable-width container. That produces exactly this "soft grid"
// look even though the source is technically sharp. shape-rendering:
// crispEdges is the SVG equivalent of image-rendering:pixelated for
// raster images -- it tells the renderer to snap edges to pixel
// boundaries instead. Injected directly into the svg tag itself (not
// relying on CSS on whatever ends up displaying it) so it's baked in
// however this string is later used -- shown live inline, loaded as an
// <img> via a data URI, or drawn to a canvas for the grid download,
// all of which rasterize this same string independently.
...
async function _getRawSvgForDownload(id){
  ...
  try{
    ...
    // Lazy-image collections: the live source is a /db/token-image URL, not
    // SVG text -- fetch the SVG so No-BG (which edits the SVG itself) gets
    // exactly what the inline format used to provide.
    if(typeof _isServedSvgUrl === 'function' && _isServedSvgUrl(liveSrc)){
      const r = await fetch(liveSrc);
      const text = r.ok ? (await r.text()).trim() : '';
      const fetched = _svgTextFromAny(text) || (text.startsWith('<svg') ? text : null);
      if(fetched){ _rememberRawSvgForDownload(id, fetched); return fetched; }
    }
  }catch(_){ }
...
// (_isServedSvgUrl lives in app.js -- this file is lazy-loaded, so the token
// modal's SVG-button check can't depend on anything defined here.)
...
async function downloadTokenSvg(id){
  ...
  try{
    // Lazy-image collections: the source is a /db/token-image URL (&fmt=svg
    // marks SVGs) rather than inline SVG text -- fetch the text to save it.
    let src = await _getTokenDownloadSource(id);
    if(_isServedSvgUrl(src)){
      try{ const r = await fetch(src); src = r.ok ? (await r.text()).trim() : null; }catch(_){ src = null; }
    }
    const s = _svgCrisp(src);
