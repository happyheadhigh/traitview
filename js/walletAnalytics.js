...
// jv: "It's now recognizing my other wallet. But no holdings data." --
// client-side merge for OCAS's separate, older backend, which each
// wallet is fetched from individually rather than via the newer
// backend's comma-separated aggregation. These mirror the exact same
// combining logic api.js already does server-side for the newer
// backend (SUM for counts, MIN for best_rank since lower is better,
// etc.) so the result reads identically either way.
function _mwRank(t){ const r = t?.os_rank ?? t?.obs_rank; return (r == null ? 999999 : Number(r)); }
function mergeWalletSummaries(responses, address){
  const valid = (responses || []).filter(r => r?.ok !== false && r?.summary);
  if(!valid.length) return responses?.[0] || { ok:false, error:'wallet summary unavailable' };
  const sums = valid.map(r => r.summary);
  const ownedCount = sums.reduce((s,x) => s + (Number(x.owned_count) || 0), 0);
  const listedCount = sums.reduce((s,x) => s + (Number(x.listed_count) || 0), 0);
  const realizedPnl = sums.reduce((s,x) => s + (Number(x.realized_pnl) || 0), 0);
  const soldCount = sums.reduce((s,x) => s + (Number(x.sold_count) || 0), 0);
  const ranks = sums.map(x => x.best_rank).filter(r => r != null).map(Number);
  const bestRank = ranks.length ? Math.min(...ranks) : null;
  const floorVals = sums.map(x => x.estimated_floor_value).filter(v => v != null).map(Number);
  const estimatedFloorValue = floorVals.length ? floorVals.reduce((a,b)=>a+b,0) : null;
  const floorEth = sums.map(x => x.floor_eth).find(v => v != null) ?? null;
  const totalCostBasisRaw = sums.reduce((s,x) => s + (Number(x.total_cost_basis) || 0), 0);
  const totalCostBasis = totalCostBasisRaw > 0 ? totalCostBasisRaw : null;
  const unrealizedPnl = (estimatedFloorValue != null && totalCostBasis > 0) ? (estimatedFloorValue - totalCostBasis) : null;
  const seenIds = new Set();
  const topTokens = sums.flatMap(x => x.top_tokens || [])
    .filter(t => { const id = Number(t.token_id); if(seenIds.has(id)) return false; seenIds.add(id); return true; })
    .sort((a,b) => _mwRank(a) - _mwRank(b))
    .slice(0, 250);
  return {
    ok:true, address, synced:true, cached:false,
    summary: {
      owned_count: ownedCount, best_rank: bestRank, listed_count: listedCount,
      estimated_floor_value: estimatedFloorValue, floor_eth: floorEth,
      realized_pnl: realizedPnl, sold_count: soldCount,
      unrealized_pnl: unrealizedPnl, total_cost_basis: totalCostBasis,
      top_tokens: topTokens,
    },
  };
}
function mergeWalletTraits(responses, address){
  const valid = (responses || []).filter(r => r?.ok !== false && Array.isArray(r?.traits));
  if(!valid.length) return responses?.[0] || { ok:false, error:'wallet traits unavailable' };
  const byKey = new Map();
  for(const r of valid){
    for(const t of r.traits){
      const key = `${t.trait_name}\u0000${t.trait_value}`;
      const existing = byKey.get(key);
      if(existing){
        existing.count += Number(t.count) || 0;
        existing.token_ids = [...new Set([...existing.token_ids, ...(t.token_ids || [])])];
      }else{
        byKey.set(key, { trait_name: t.trait_name, trait_value: t.trait_value, count: Number(t.count) || 0, token_ids: [...(t.token_ids || [])] });
      }
    }
  }
  const traits = [...byKey.values()].sort((a,b) => b.count - a.count || String(a.trait_name).localeCompare(b.trait_name) || String(a.trait_value).localeCompare(b.trait_value));
  return { ok:true, address, synced:true, traits, count: traits.length };
}
function mergeWalletHistory(responses, address){
  const valid = (responses || []).filter(r => r?.ok !== false && Array.isArray(r?.history));
  if(!valid.length) return responses?.[0] || { ok:false, error:'wallet history unavailable' };
  const byDate = new Map();
  for(const r of valid){
    for(const h of r.history){
      const key = String(h.snapshot_date);
      const existing = byDate.get(key);
      const ownedCount = Number(h.owned_count) || 0;
      const listedCount = Number(h.listed_count) || 0;
      const floorVal = h.estimated_floor_value != null ? Number(h.estimated_floor_value) : null;
      if(existing){
        existing.owned_count += ownedCount;
        existing.listed_count += listedCount;
        if(floorVal != null) existing.estimated_floor_value = (existing.estimated_floor_value || 0) + floorVal;
        if(h.best_rank != null && (existing.best_rank == null || h.best_rank < existing.best_rank)) existing.best_rank = h.best_rank;
      }else{
        byDate.set(key, { snapshot_date: h.snapshot_date, owned_count: ownedCount, listed_count: listedCount, estimated_floor_value: floorVal, best_rank: h.best_rank ?? null });
      }
    }
  }
  const history = [...byDate.values()].sort((a,b) => new Date(a.snapshot_date) - new Date(b.snapshot_date));
  const days = valid.map(r => r.days).find(d => d != null) ?? null;
  return { ok:true, address, synced:true, history, count: history.length, days };
}
function mergeWalletTransfers(responses, address){
  const valid = (responses || []).filter(r => r?.ok !== false && Array.isArray(r?.transfers));
  if(!valid.length) return responses?.[0] || { ok:false, error:'wallet transfers unavailable' };
  const transfers = valid.flatMap(r => r.transfers)
    .sort((a,b) => new Date(b.block_ts || 0) - new Date(a.block_ts || 0))
    .slice(0, 500);
  const contract = valid.map(r => r.contract).find(c => c != null) ?? null;
  return { ok:true, address, contract, synced:true, transfers, count: transfers.length, limit:500, offset:0 };
}
...
async function loadWalletAnalytics(address, opts={}){
  ...
  try{
    ...
    const others = isConnectedWallet
      ...
      : [];
    // jv: "It's now recognizing my other wallet. But no holdings data."
    // -- HTTP 400 on OCAS specifically. Root cause: the comma-separated
    // multi-address format above was only ever built/supported against
    // the newer tv-bot-api-production backend (api.js there). OCAS's own
    // apiBase is a genuinely separate, older, standalone service (see the
    // COLLECTIONS registry comment in config.js) that was never upgraded
    // to parse it -- an HTTP 400 (not a 404) fits: the route exists, it
    // just doesn't recognize the comma-joined string as a valid address.
    // Not fixable from this repo alone since it's a separate backend's
    // own request validation, so doing the combining client-side instead
    // for OCAS specifically: each wallet fetched on its own (still a
    // single, valid address each -- exactly what that backend already
    // handles fine), then merged here the same way api.js's own SQL
    // already aggregates for the newer backend (summed counts, MIN best
    // rank, combined+re-sorted top tokens, etc.), so the result reads the
    // same either way.
    const supportsMultiWallet = (typeof LIVE_SLUG !== 'undefined' ? LIVE_SLUG : null) !== 'on-chain-all-stars';
    let summary, traits, history, transfers, addrParam;
    if(others.length && !supportsMultiWallet){
      addrParam = [addr, ...others].join(',');
      const perWallet = await Promise.all([addr, ...others].map(async w => {
        const wEnc = encodeURIComponent(w);
        const [s, t, h, tr] = await Promise.all([
          dbFetch(`/db/wallet/${wEnc}/summary`).catch(e => ({ ok:false, error:e.message })),
          dbFetch(`/db/wallet/${wEnc}/traits`).catch(e => ({ ok:false, error:e.message })),
          dbFetch(`/db/wallet/${wEnc}/history`).catch(e => ({ ok:false, error:e.message })),
          dbFetch(`/db/wallet/${wEnc}/transfers`, { limit: 500 }).catch(e => ({ ok:false, error:e.message }))
        ]);
        return { s, t, h, tr };
      }));
      summary = mergeWalletSummaries(perWallet.map(p => p.s), addr);
      traits = mergeWalletTraits(perWallet.map(p => p.t), addr);
      history = mergeWalletHistory(perWallet.map(p => p.h), addr);
      transfers = mergeWalletTransfers(perWallet.map(p => p.tr), addr);
    }else{
      addrParam = others.length ? [addr, ...others].join(',') : addr;
      const enc = encodeURIComponent(addrParam);
      [summary, traits, history, transfers] = await Promise.all([
...
function walletTimelineTraceHtml(hostId, days, seriesDefs, comparison){
  ...
  setTimeout(() => {
    const render = () => {
      ...
      if(!el) return; if(typeof Plotly === 'undefined'){ window.ensurePlotly && window.ensurePlotly(); setTimeout(render, 80); return; }
