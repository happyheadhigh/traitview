...
function burnsTokenImgSrc(id){
  ...
  // missing _svgCrisp() fix as _getTokenImgSrc's identical dbImg branch
  // in app.js -- a separate, parallel implementation for the burns
  // analytics panel specifically.
  if(s && s.startsWith('<svg')){
    try{ return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(_svgCrisp(s)); }catch(_){ return ''; }
  }
...
function drawBurnActivityChart(rows){
  ...
  if(typeof Plotly === 'undefined'){ window.ensurePlotly && window.ensurePlotly(); setTimeout(() => drawBurnActivityChart(rows), 80); return; }
