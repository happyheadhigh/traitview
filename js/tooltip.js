...
/* tooltip + modal */
function tipHtml(id,row){ const kv=keepEntries(row.traits); const traitsHtml=kv.length?kv.map(([k,v])=>`<div><span>${traitDisplayLabel(k)}</span><b>${v}</b></div>`).join(''):'<div style="color:var(--muted)">No traits</div>'; let imgHtml='<div style="color:var(--muted)">No image</div>'; const mapVal=(typeof LIVE_SLUG!=='undefined' && LIVE_SLUG==='on-chain-all-stars') ? (IMAGES_MAP && IMAGES_MAP.get(id)) : null; const src=row.image || mapVal || (typeof _getTokenImgSrc==='function' ? _getTokenImgSrc(id) : null); if(src){ const s=String(src).trim(); if(s.startsWith('<svg')) imgHtml=`<div class="svg-wrap">${s}</div>`; else if(/^data:image\//i.test(s)) imgHtml=`<img src="${s}" alt="#${id}">`; else imgHtml=`<img src="${ipfsToHttp(s)}" alt="#${id}">`; } const listing=(window.LISTINGS&&window.LISTINGS[id]&&window.LISTINGS[id].opensea)||null; const ethVal = parseEthMaybeWei(listing?listing.price:null); const linkHtml=(listing&&listing.url&&ethVal!=null)?`<a class="linkpill" href="${listing.url}" target="_blank" rel="noopener">OpenSea (${formatEth(ethVal, listing?.currency)})</a>`:''; const rankHtml=(typeof displayRankHtml==='function'&&displayRankHtml(id))?`<span class="chip">${displayRankHtml(id)}</span>`:''; return `<div class="tip-title">#${id} • Traits: ${getTraitCount(row)} ${rankHtml}</div>${linkHtml?`<div style="margin:6px 0">${linkHtml}</div>`:''}<div class="tip-body"><div class="tip-img">${imgHtml}</div><div class="traits">${traitsHtml}</div></div>`; }
function attachPreviewHandlers(){
  ...
  const isTouchDevice = () => window.matchMedia('(hover: none)').matches || window._tvIsPhone();
