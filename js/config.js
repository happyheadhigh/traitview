...
function resetCollectionState(){
  ...
  // per-collection Gondi trades (Sale Chart wallet panel) + holders request flag
  window._gondiTrades = null; window._gondiTradesRequested = false; window._scHoldersRequested = false;
  window._holdersData = null;
  window._holderStats = null;
...
// just look broken.
// jv: "the banners are still stretched and blurry on desktop." The
// console log confirmed the actual cause: OpenSea's image CDN shards
// across multiple subdomains (i.seadn.io, i2c.seadn.io, seen live
// here, and likely others) -- this only ever matched the exact "i."
// prefix, so it silently failed to match i2c.seadn.io and passed the
// URL through completely unchanged every time, which is exactly why
// the earlier fix never visibly helped at all. Widened to match any
// subdomain of seadn.io, not just that one exact one.
function upgradeSeadnImageResolution(url, width){
  if(!url || !/https?:\/\/[a-z0-9]+\.seadn\.io\//i.test(url)) return url;
  try{
    const u = new URL(url);
...
function applyCollectionBannerHeader(){
  ...
  const bannerBgEl = document.getElementById('collectionBannerBg');
  const avatarEl = document.getElementById('collectionHeaderAvatarImg');
  const nameEl = document.getElementById('collectionHeaderName');
  if(!header || !bannerEl || !avatarEl) return;
  const entry = COLLECTIONS[LIVE_SLUG];
  const bannerUrl = upgradeSeadnImageResolution(entry?.bannerImageUrl || null, 1920);
  const avatarUrl = upgradeSeadnImageResolution(entry?.avatarImageUrl || null, 400);
  if(!bannerUrl && !avatarUrl){
    header.style.display = 'none';
    return;
  }
  // jv: relayed a proposed fix for the banner sizing back-and-forth --
  // a blurred, cover-sized backdrop layer behind the real, uncropped
  // (object-fit:contain) artwork on top, rather than one image forced
  // to be both. Both layers use the exact same URL/asset, just fit
  // differently; an empty src shows a broken-image icon in most
  // browsers, so hiding both entirely rather than setting src="" for
  // the no-banner case.
  if(bannerUrl){
    bannerEl.src = bannerUrl;
    bannerEl.style.display = '';
    if(bannerBgEl){
      bannerBgEl.src = bannerUrl;
      bannerBgEl.style.display = '';
    }
  }else{
  ...
  // jv: "can we add the collection names right under the banner image
  // and to the right of the avatar." entry.name is the same display
  // name already used for this collection's own dropdown option.
