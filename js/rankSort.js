...
function applyViewMode(val){
  ...
  // jv: "can I get the 2x2 grid back as well?" -- view-2x2 already means
  // the 3x3 mode (val === 'standard') by this point, from the earlier
  // 2->3 column change; renaming it everywhere it's used in CSS would be
  // a wide, risky touch for no real benefit since the class name itself
  // isn't user-facing. New, separate class for the true 2x2 mode instead.
  tg.classList.toggle('view-2x2', val === 'standard');
  tg.classList.toggle('view-grid2', val === 'grid2');
  tg.classList.toggle('view-grid1', val === 'grid1');
  tg.classList.toggle('view-5x5', val === 'grid5');

  if(val === 'compact' || val === 'standard' || val === 'grid5' || val === 'grid2' || val === 'grid1'){
    const mobileGrid = window._tvIsPhone();
